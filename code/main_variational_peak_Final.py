...
def print_forecast_table(mu_seq, target_seq=None, scaler=None, hours=48, timestamps=None,
    ...
    if has_target:
        ...
        peak_pred = np.max(mu_np)
        peak_actual = np.max(tgt_np)
        ...
        print(f"{'-' * width}")
        print(f"{'Peak':>{ts_w}s}  {np.max(mu_np):>12.4f}")
...
def save_forecast_table_png(mu_np, tgt_np, ts_labels, save_path="forecast_48h.png",
    ...
    if title is None:
        title = f"{len(mu_np)}-Hour Load Forecast"

    cmap = plt.cm.RdYlGn_r
    norm = mcolors.Normalize(vmin=0, vmax=30)

    mid = min(24, len(mu_np))
    h1_pred, h2_pred   = mu_np[:mid],      mu_np[mid:]
    h1_true, h2_true   = tgt_np[:mid],     tgt_np[mid:]
...
if __name__ == "__main__":
    ...
    # Compute timestamps for the last test sample's decoder period
    last_decoder_week_ts = times[-1] if len(times) > 0 else None
    ...
    evaluate_model(model_hmu_eval, test_loader, nn.MSELoss(), device,
                   ...
                   forecast_hours=48,
                   forecast_timestamps=last_decoder_week_ts)
